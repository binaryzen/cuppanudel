import { run, assert, assertNull } from '../test/runner.js';
import { createMediaPool } from './media-pool.js';

run('pool/media-pool-minor', async (test) => {
  ...
  test("getBuffer('nonexistent') returns undefined (confirming existing behaviour)", () => {
    ...
    assertNull(result, 'getBuffer should return undefined for nonexistent id');
  });
